...
- 🖐️ **양손 지원**: 왼손·오른손 각각 검지/중지/약지/소지에 서로 다른 단어를 설정 가능 (왼손 검지 제외 총 7개)
- 🤏 **핀치 제스처**: 엄지와 손가락 끝을 튕기면(pinch) 해당 단어를 음성으로 읽어줌
- 🖼️ **SSAFY 로고 슬롯**: 왼손 검지는 고정 슬롯으로, 핀치하면 SSAFY 로고 이미지와 전용 음성 클립(`ssafy-voice.mp3`)이 재생되고 카메라와의 거리에 따라 이미지 크기가 확대/축소됨
- 👍 **엄지척 제스처 배너**: 주먹을 쥐고 엄지를 세우면 화면에 "I need SSAFY" 배너가 표시됨
- 🎨 **손가락별 파스텔 컬러**: 민트(검지) · 레몬(중지) · 핑크(약지) · 라벤더(소지) 텍스트 색상, 배경 박스 없이 텍스트만 표시
- 🗣️ **음성 커스터마이즈**: 기기에 설치된 한국어 음성 목록 중 선택 가능 (남성 음성 자동 우선 선택, 여성 음성은 피치 자동 보정)
- ✨ **화면 보정 / 무음 모드**: 뽀샤시 필터 On/Off, 소리 없이 단어만 표시하는 무음 모드 지원
- 🔤 **폰트 선택**: Paperlogy / Pretendard 중 선택 가능 (localStorage에 저장)
...
├── app.js           # 손 인식, 핀치 감지, 제스처 인식, 음성 재생 로직
├── ssafy.png        # 왼손 검지 고정 슬롯 이미지
├── ssafy-voice.mp3  # 왼손 검지 핀치 시 재생되는 음성 클립
└── README.md
```
